...
export default function CreatePost() {
  ...
  const saveDraftToDB = useCallback(async ({ redirectAfter = false, silent = false } = {}) => {
    ...
    try {
      ...
      const data = await res.json();
      // console.log("✅ Draft saved:", data); // Not required — draft save confirmation, remove before production
  ...
  useEffect(() => {
    ...
    const dbTimer = setTimeout(() => {
      if (!isPublishingRef.current) saveDraftToDB({ silent: true });
    }, 3000);
  ...
  try {
    // If thumbnail is still a local base64, upload it first and get a URL back.
    // This keeps multi-MB base64 strings out of the DB and post JSON payloads.
    ...
    const data = await res.json();
    // console.log("✅ Response:", data); // Not required — publish response log, remove before production
    clearDraft();
...
const parentOptions = [
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full overflow-x-hidden">
      ...
      {draftBanner && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl px-6 py-4 shadow-sm">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={restoreDraft} className="inline-flex items-center gap-2 px-4 py-3 text-sm sm:text-base rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-semibold transition"><FaHistory size={12} /> Restore Draft</button>
            <button onClick={discardDraft} className="inline-flex items-center gap-2 px-4 py-3 text-sm sm:text-base rounded-xl bg-white dark:bg-slate-700 hover:bg-amber-100 dark:hover:bg-slate-600 border border-amber-200 dark:border-amber-700 text-amber-700 dark:text-amber-300 font-semibold transition"><FaTrashAlt size={12} /> Discard</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-sm sm:text-base font-semibold px-4 py-3 rounded-2xl">{wordCount} words</span>
                <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-sm sm:text-base font-semibold px-4 py-3 rounded-2xl inline-flex items-center gap-2"><FaBookOpen /> {readingTime} min</span>
              </div>
            </div>
  ...
  type="button"
  onClick={() => {
    // console.log("🔥 FINAL CLICK"); // Not required — click confirmation debug log
    handlePublish();
  }}
